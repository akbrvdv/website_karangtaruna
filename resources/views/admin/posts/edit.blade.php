    ...
    <div class="max-w-4xl mx-auto">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="p-6 border-b border-gray-100">
                <h2 class="text-lg font-bold text-gray-800">Perbarui Publikasi</h2>
                <p class="text-sm text-gray-500">Edit informasi berita yang sudah dipublikasikan sebelumnya.</p>
            </div>
            ...
            <form action="{{ route('admin.posts.update', $post->id) }}" method="POST" class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea id="content" name="content" rows="8" required 
                        class="mt-1 block w-full rounded-xl border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm px-4 py-3 transition">{{ old('content', $post->content) }}</textarea>
                    @error('content')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
